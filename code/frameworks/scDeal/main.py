...
from training_utils import run_scdeal_benchmark
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train and evaluate scDEAL using custom source/target datasets."
    )
    ...
    parser.add_argument("--epochs-bulk", type=int, default=50)
    parser.add_argument("--epochs-sc", type=int, default=100)
    parser.add_argument("--batch-size", type=int, default=32)
    parser.add_argument("--lr", type=float, default=1e-2)
    parser.add_argument("--lr-bulk", type=float, default=1e-2)
    parser.add_argument("--lr-sc", type=float, default=1e-2)
    parser.add_argument("--dropout", type=float, default=0.3)
    parser.add_argument("--mmd-weight", type=float, default=0.25)
    parser.add_argument("--regularization-weight", type=float, default=1.0)
    parser.add_argument("--bottleneck", type=int, default=128)
    parser.add_argument("--bulk-h-dims", default="512,256", help="Comma separated hidden layer sizes for the bulk encoder.")
    parser.add_argument("--predictor-h-dims", default="64,32", help="Comma separated hidden layer sizes for the predictor head.")
    parser.add_argument(
        "--freeze-pretrain",
        type=int,
        default=0,
        help="If non-zero, freeze pretrained encoder weights during adaptation.",
    )
    parser.add_argument(
        "--balancing-strategy",
        default="weighted",
        choices=["none", "weighted", "smote"],
        help="Class-balancing strategy for source and target training loaders.",
    )
    ...
    parser.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Enable threshold tuning on validation data.",
    )
    parser.add_argument(
        "--binarize-source",
        action=argparse.BooleanOptionalAction,
...
def main() -> None:
    ...
    benchmark_args = {
        "drug": args.drug,
        "epochs": args.epochs,
        "epochs_bulk": args.epochs_bulk,
        "epochs_sc": args.epochs_sc,
        "batch_size": args.batch_size,
        "lr": args.lr,
        "lr_bulk": args.lr_bulk,
        "lr_sc": args.lr_sc,
        "dropout": args.dropout,
        "mmd_weight": args.mmd_weight,
        "regularization_weight": args.regularization_weight,
        "bottleneck": args.bottleneck,
        "bulk_h_dims": args.bulk_h_dims,
    ...
    results = run_scdeal_benchmark(
        ...
        seed=args.seed,
        x_val_target=prepared["x_val_target"],
        y_val_target=prepared["y_val_target"],
    )
