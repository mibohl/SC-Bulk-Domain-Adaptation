...
def process_116237(adata, **kargs):
    ...
    for section in [0, 1, 2]:
        svals = [re.split(r"_|\.", index)[section] for index in obs_names]
        annotation_dict["name_section_" + str(section + 1)] = svals
...
def plot_training_history(file_path, save_path, loss_type="Loss"):
    """
    Plots the training and validation loss from a training log file.

    Args:
        file_path (str): The path to the training log file.
        save_path (str): The path to save the plot.
        loss_type (str): The name of the loss column in the log file.
    """
    import matplotlib.pyplot as plt
    import pandas as pd
    ...
    # Plot Loss
    plt.plot(train_df["Epoch"], train_df[loss_type], label=f"Train {loss_type}")
    plt.plot(val_df["Epoch"], val_df[loss_type], label=f"Validation {loss_type}")
...
def mmd_loss(x_src, x_tar, gamma=1000):
