...
class AEBase(nn.Module):
    def __init__(self, input_dim, latent_dim=128, h_dims=[512], drop_out=0.3):
        ...
        self.final_layer = nn.Sequential(nn.Linear(hidden_dims[-2], hidden_dims[-1]), nn.Sigmoid())
...
class Predictor(nn.Module):
    def __init__(self, input_dim, output_dim=1, h_dims=[512], drop_out=0.3):
        ...
        self.output = nn.Sequential(nn.Linear(hidden_dims[-1], output_dim), nn.Sigmoid())
    ...
    def __init__(
        ...
        if freezed:
            ...
            for p in self.parameters():
                if bottlenect_reached and (p.shape.numel() > self.latent_dim):
                    break
...
class CVAEBase(VAEBase):
    ...
    def __init__(self, input_dim, n_conditions, latent_dim=128, h_dims=[512], drop_out=0.3):
